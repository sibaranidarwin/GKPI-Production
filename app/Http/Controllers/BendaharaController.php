...
class BendaharaController extends Controller
{
    ...
    function datakeluarga(Request $request)
    {
        ...
        return view('bendahara.datakeluarga', [
    ...
    function formkeluarga(Request $request)
    {
        ...
        return view("bendahara.formkeluarga");
    }
    function formkeluargaprocess(Request $request)
    {
        ...
        return redirect()->route('bendahara.datakeluarga')->with('success', 'Data Keluarga Sudah Berhasil Ditambahkan');
    }
    ...
    function detailkeluarga(Request $request, $id)
    {
        ...
        return view('bendahara.detailkeluarga', ["keluarga" => $keluarga, 'lampiran' => $lampiran,  'jemaats' => $jemaat]);
    }
    public function editdatakeluarga($id)
    {
        ...
        $lampiran = explode("#", $keluarga['lampiran']);
        $jemaat = DB::select('Select j.*, jk.status, s.nama as sektor_name FROM jemaat j INNER JOIN jemaat_keluarga jk ON j.nik = jk.nik INNER JOIN sektor s ON s.id =j.sektor_id WHERE jk.no_kk = ?', [$id]);
        return view('bendahara.editdatakeluarga', ["keluarga" => $keluarga, 'lampiran' => $lampiran,  'jemaats' => $jemaat]);
    }
    ...
    function datajemaat(){
        ...
        return view('bendahara.datajemaat', [
    ...
    function formjemaat(Request $request, $idKeluarga)
    {
        ...
        return view('bendahara.formjemaat', ['sektors' => $sektors, 'keluarga' => $keluarga]);
    }
    ...
    function formjemaatprocess(Request $request, $idKeluarga)
    {
        ...
        if (!$jemaat->save()) {
            ...
            return redirect('/bendahara/data/keluarga/' . $idKeluarga)->with('success', 'Data Jemaat Berhasil Disimpan!');
        }
    }
    ...
    function detailjemaat(Request $request, $nik){
        ...
        return view('bendahara.detailjemaat', ['jemaat'=> $jemaat,'lampiran'=> $lampiran]);
    }
    function editdetailjemaat(Request $request, $nik){
        $jemaat = Jemaat::where("nik", $nik)->first();
        $lampiran = explode("#", $jemaat['lampiran']);
        return view('bendahara.editdatajemaat', ['jemaat'=> $jemaat,'lampiran'=> $lampiran]);
    }
 ...
 $jemaat = Jemaat::all();
        ...
        return view('bendahara.datastatistik', [
    ...
    function pelayan()
    {
        ...
        return view('bendahara.datapelayan', [
            "pelayanas" => $pelayan
        ]);   
    }
      function baktikeluarga(Request $request){
        
        $no_kk = Keluarga::get();
        return view("bendahara.tambahbaktikeluarga", ["no_kks" => $no_kk]);
    }
    function baktijemaat(Request $request){
        
        $nik = Jemaat::get();
        return view("bendahara.tambahbaktijemaat", ["niks" => $nik]);
    }
   
    function tambahbaktikeluarga(Request $request){
        $data = $request->validate([
            'no_kk'     => ['required'],
            'nama_keluarga'    => ['required'],
            'bakti_bulan'   => ['required'],
            'bakti_pembangunan'   => ['required'],
        ...
        Bakti_keluarga::create($data);
        
        return redirect()->route('bendahara.baktikeluarga')->with('success','Bakti Bulanan dan Pembangunan Keluarga ditambahkan!');

    }
    function tambahbaktijemaat(Request $request){
        $data = $request->validate([
            'nik'     => ['required'],
            'nama'    => ['required'],
            'bakti_bulan'   => ['required'],
        ...
        Bakti_jemaat::create($data);
        
        return redirect()->route('bendahara.baktijemaat')->with('success','Bakti Bulanan dan pembangunan Jemaat ditambahkan!');

    }
    function formpelayan(Request $request)
    {

        return view("bendahara.formdatapelayan");
    }
    function formpelayanprocess(Request $request)
    {
        ...
        return redirect('/bendahara/pelayangereja')->with('success', 'Data Pelayan Berhasil Disimpan!');
    }
}